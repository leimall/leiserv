...
func RollGormBack(pk, model string) {
	...
	// 如果多个 仅仅删除对应块即可
	// 如果单个 那么还需要剔除import
	path := filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Server, "initialize", "gorm_biz.go")
			...
			if gen, ok := node.(*ast.GenDecl); ok {
				for i := range gen.Specs {
					if imspec, ok := gen.Specs[i].(*ast.ImportSpec); ok {
						if imspec.Path.Value == "\"leiserv/models/"+pk+"\"" {
							gp = gen
							imI = i
							return false
						}
...
func RollRouterBack(pk, model string) {
	...
	path := filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Server, "initialize", "router_biz.go")
	src, err := os.ReadFile(path)
	if err != nil {
		fmt.Println(err)
	}
