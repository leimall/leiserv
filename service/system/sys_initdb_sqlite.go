...
func (h SqliteInitHandler) WriteConfig(ctx context.Context) error {
	...
	global.MALL_CONFIG.System.DbType = "sqlite"
	global.MALL_CONFIG.Sqlite = c
	global.MALL_CONFIG.JWT.SigningKey = uuid.Must(uuid.NewV4()).String()
	cs := utils.StructToMap(global.MALL_CONFIG)
	for k, v := range cs {
		global.MALL_VP.Set(k, v)
	}
	return global.MALL_VP.WriteConfig()
}
...
func (h SqliteInitHandler) EnsureDB(ctx context.Context, conf *request.InitDB) (next context.Context, err error) {
	...
	global.MALL_CONFIG.AutoCode.Root, _ = filepath.Abs("..")
