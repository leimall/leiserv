...
import (
	...
	ast2 "leiserv/utils/ast"

	"leiserv/resource/autocode_template/subcontract"
...
func Init(Package string) {
	injectionPaths = []injectionMeta{
		{
			path: filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SApi, Package), "enter.go"),
			funcName:    "ApiGroup",
			structNameF: "%sApi",
		},
		{
			path: filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SRouter, Package), "enter.go"),
			funcName:    "RouterGroup",
			structNameF: "%sRouter",
		},
		{
			path: filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SService, Package), "enter.go"),
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			path: filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, "service", "enter.go"),
			importCodeF:  "leiserv/%s/%s",
			packageNameF: "%s",
		...
		packageRouterName: {
			path: filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, "router", "enter.go"),
			importCodeF:  "leiserv/%s/%s",
			packageNameF: "%s",
		...
		packageAPIName: {
			path: filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, "api/v1", "enter.go"),
			importCodeF:  "leiserv/%s/%s",
			packageNameF: "%s",
			groupName:    "ApiGroup",
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system.AutoCodeStruct, menuID uint, ids ...uint) (err error) {
...

	{
		if autoCode.AutoMigrate {
			// 在gorm.go 注入 自动迁移
			path := filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, global.MALL_CONFIG.AutoCode.SInitialize, "gorm_biz.go")
...

	{
		// router.go 注入 自动迁移
		path := filepath.Join(global.MALL_CONFIG.AutoCode.Root,
			global.MALL_CONFIG.AutoCode.Server, global.MALL_CONFIG.AutoCode.SInitialize, "router_biz.go")
...
func (autoCodeService *AutoCodeService) DropTable(BusinessDb, tableName string) error {
	if BusinessDb != "" {
		...
		return global.MALL_DB.Exec("DROP TABLE " + tableName).Error
	}
}
...
func (autoCodeService *AutoCodeService) addAutoMoveFile(data *tplData) {
	...
	if strings.Contains(fileSlice[1], "server") {
		if strings.Contains(fileSlice[n-2], "router") {
			data.autoMoveFilePath = filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Server,
				fmt.Sprintf(global.MALL_CONFIG.AutoCode.SRouter, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "api") {
			data.autoMoveFilePath = filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SApi, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "service") {
			data.autoMoveFilePath = filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SService, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "model") {
			data.autoMoveFilePath = filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SModel, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "request") {
			data.autoMoveFilePath = filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Server, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SRequest, data.autoPackage), base)
		}
	} else if strings.Contains(fileSlice[1], "web") {
		if strings.Contains(fileSlice[n-1], "js") {
			data.autoMoveFilePath = filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Web, global.MALL_CONFIG.AutoCode.WApi, data.autoPackage, base)
		} else if strings.Contains(fileSlice[n-2], "form") {
			data.autoMoveFilePath = filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Web, global.MALL_CONFIG.AutoCode.WForm, data.autoPackage, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), strings.TrimSuffix(base, filepath.Ext(base))+"Form.vue")
		} else if strings.Contains(fileSlice[n-2], "table") {
			data.autoMoveFilePath = filepath.Join(global.MALL_CONFIG.AutoCode.Root,
				global.MALL_CONFIG.AutoCode.Web, global.MALL_CONFIG.AutoCode.WTable, data.autoPackage, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), base)
		}
	}
}
...
func (autoCodeService *AutoCodeService) AutoCreateApi(a *system.AutoCodeStruct) (ids []uint, err error) {
	...
	err = global.MALL_DB.Transaction(func(tx *gorm.DB) error {
...
func (autoCodeService *AutoCodeService) AutoCreateMenu(a *system.AutoCodeStruct) (id uint, err error) {
	...
	err = global.MALL_DB.First(&menu, "name = ?", a.Abbreviation).Error
	if err == nil {
	...
	err = global.MALL_DB.Create(&menu).Error
	return menu.ID, err
}
...
func (autoCodeService *AutoCodeService) CreatePackageTemp(packageName string) error {
	...
	webTemp := []string{
		filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Web, global.MALL_CONFIG.AutoCode.WApi),
		filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Web, global.MALL_CONFIG.AutoCode.WForm),
	}
	...
	for i, s := range pendingTemp {
		pendingTemp[i].path = filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Server, filepath.Clean(fmt.Sprintf(s.path, packageName)))
	}
	// 选择模板
...
func (autoCodeService *AutoCodeService) createPluginServer(plug system.AutoPlugReq) error {
	...
	for _, tpl := range tplFileList {
		...
		if strings.Index(pathArr[3], "tpl") < 0 {
			dirPath := filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Server, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SPlug, plug.Snake+"/"+pathArr[3]))
			os.MkdirAll(dirPath, 0755)
		}
		file := filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Server, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SPlug, plug.Snake+"/"+tpl[len(plugServerPath):len(tpl)-4]))
...
func (autoCodeService *AutoCodeService) createPluginWeb(plug system.AutoPlugReq) error {
	...
	for _, tpl := range tplFileList {
		...
		pathArr := strings.SplitAfter(tpl, "/")
		if strings.Index(pathArr[3], "tpl") < 0 {
			dirPath := filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Web, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SPlug, plug.Snake+"/"+pathArr[3]))
			os.MkdirAll(dirPath, 0755)
		}
		file := filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Web, fmt.Sprintf(global.MALL_CONFIG.AutoCode.SPlug, plug.Snake+"/"+tpl[len(plugWebPath):len(tpl)-4]))
		f, err := os.OpenFile(file, os.O_WRONLY|os.O_CREATE, 0666)
		if err != nil {
			zap.L().Error("open file", zap.String("tpl", tpl), zap.Error(err), zap.Any("plug", plug))
			return err
...
func (autoCodeService *AutoCodeService) InstallPlugin(file *multipart.FileHeader) (web, server int, err error) {
	...
	if len(serverPlugin) != 0 {
		err = installation(serverPlugin, global.MALL_CONFIG.AutoCode.Server, global.MALL_CONFIG.AutoCode.Server)
		if err != nil {
	...
	if len(webPlugin) != 0 {
		err = installation(webPlugin, global.MALL_CONFIG.AutoCode.Server, global.MALL_CONFIG.AutoCode.Web)
		if err != nil {
			return webIndex, serverIndex, err
		}
	}
...
func installation(path string, formPath string, toPath string) error {
	...
	var form = filepath.ToSlash(global.MALL_CONFIG.AutoCode.Root + formPath + "/" + path)
	var to = filepath.ToSlash(global.MALL_CONFIG.AutoCode.Root + toPath + "/plugin/")
...
func (autoCodeService *AutoCodeService) PubPlug(plugName string) (zipPath string, err error) {
	...
	webPath := filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Web, "plugin", plugName)
	serverPath := filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Server, "plugin", plugName)
	// 创建一个新的zip文件
	...
	return filepath.Join(global.MALL_CONFIG.AutoCode.Root, global.MALL_CONFIG.AutoCode.Server, fileName), nil
}
