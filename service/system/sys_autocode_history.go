...
func (autoCodeHistoryService *AutoCodeHistoryService) CreateAutoCodeHistory(meta, structName, structCNName, autoCodePath string, injectionMeta string, tableName string, apiIds string, Package string, BusinessDB string, menuID uint) error {
	return global.MALL_DB.Create(&system.SysAutoCodeHistory{
		Package:       Package,
		RequestMeta:   meta,
...
func (autoCodeHistoryService *AutoCodeHistoryService) First(info *request.GetById) (string, error) {
	var meta string
	return meta, global.MALL_DB.Model(system.SysAutoCodeHistory{}).Select("request_meta").Where("id = ?", info.Uint()).First(&meta).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) Repeat(businessDB, structName, Package string) bool {
	var count int64
	global.MALL_DB.Model(&system.SysAutoCodeHistory{}).Where("business_db = ? and struct_name = ? and package = ? and flag = 0", businessDB, structName, Package).Count(&count)
	return count > 0
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) RollBack(info *systemReq.RollBack) error {
	...
	if err := global.MALL_DB.Where("id = ?", info.ID).First(&md).Error; err != nil {
		return err
	}
	// 清除API表
	var err error
	if info.DeleteApi {
		...
		if err != nil {
			global.MALL_LOG.Error("ClearTag DeleteApiByIds:", zap.Error(err))
		}
	}
	// 清除菜单表
	if info.DeleteMenu {
		...
		if err != nil {
			global.MALL_LOG.Error("ClearTag DeleteBaseMenu:", zap.Error(err))
		}
	}
	...
	if info.DeleteTable {
		if err = AutoCodeServiceApp.DropTable(md.BusinessDB, md.TableName); err != nil {
			global.MALL_LOG.Error("ClearTag DropTable:", zap.Error(err))
		}
	}
	// 删除文件
	...
	for _, path := range strings.Split(md.AutoCodePath, ";") {
		...
		// 迁移
		nPath := filepath.Join(global.MALL_CONFIG.AutoCode.Root,
		...
		if err != nil {
			global.MALL_LOG.Error("file move err ", zap.Error(err))
		}
	...
	md.Flag = 1
	return global.MALL_DB.Save(&md).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) Delete(info *request.GetById) error {
	return global.MALL_DB.Where("id = ?", info.Uint()).Delete(&system.SysAutoCodeHistory{}).Error
}
...
func (autoCodeHistoryService *AutoCodeHistoryService) GetList(info request.PageInfo) (list []response.AutoCodeHistory, total int64, err error) {
	...
	db := global.MALL_DB.Model(&system.SysAutoCodeHistory{})
