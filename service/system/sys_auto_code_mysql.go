...
func (s *autoCodeMysql) GetDB(businessDB string) (data []response.Db, err error) {
	...
	if businessDB == "" {
		err = global.MALL_DB.Raw(sql).Scan(&entities).Error
	} else {
		err = global.MALL_DBList[businessDB].Raw(sql).Scan(&entities).Error
	}
	return entities, err
}
...
func (s *autoCodeMysql) GetTables(businessDB string, dbName string) (data []response.Table, err error) {
	...
	if businessDB == "" {
		err = global.MALL_DB.Raw(sql, dbName).Scan(&entities).Error
	} else {
		err = global.MALL_DBList[businessDB].Raw(sql, dbName).Scan(&entities).Error
	}
...
WHERE 
    ...
    AND c.TABLE_SCHEMA = ?;`
	if businessDB == "" {
		err = global.MALL_DB.Raw(sql, tableName, dbName).Scan(&entities).Error
	} else {
		err = global.MALL_DBList[businessDB].Raw(sql, tableName, dbName).Scan(&entities).Error
	}
