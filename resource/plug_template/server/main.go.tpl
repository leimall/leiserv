...
import (
	gvaGlobal "leiserv/global"
	"leiserv/models/system"
	"leiserv/plugin/plugin-tool/utils"
{{- if .HasGlobal }}
	"leiserv/plugin/{{ .Snake}}/global"
{{- end }}
	"leiserv/plugin/{{ .Snake}}/router"
...
{{ end }}
    ...
    gvaGlobal.MALL_DB.AutoMigrate() // 此处可以把插件依赖的数据库结构体自动创建表 需要填写对应的结构体
