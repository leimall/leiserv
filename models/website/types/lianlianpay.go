...
type LianLianPay struct {
	...
	Customer              LLPayCustomer    `json:"customer,omitempty"`      // 客户详情
	PaymentData           LLPayPaymentData `json:"payment_data,omitempty"`  // 支付数据
}
...
type MerchantOrder struct {
	...
	Products          []LLPayProduct `json:"products,omitempty"`  // 订单中的商品列表
}
...
type LLPayProduct struct {
	...
	Quantity         uint    `json:"quantity"`                    // 商品数量
	Category         string  `json:"category,omitempty"`          // 商品分类
	Sku              string  `json:"sku,omitempty"`               // 商品SKU
	Url              string  `json:"url,omitempty"`               // 商品网址
	ShippingProvider string  `json:"shipping_provider,omitempty"` // 物流供应商
}
...
type LLPayPaymentData struct {
	...
	Card         LLPayCard `json:"card,omitempty"` // 信用卡信息
}
...
type LLPayCard struct {
	...
	BillingAddress LLPayBillingAddress `json:"billing_address,omitempty"` // 账单地址
}
