...
func (m *mongo) Initialization() error {
	...
	if global.MALL_CONFIG.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	client, err := qmgo.Open(ctx, &qmgo.Config{
		Uri:              global.MALL_CONFIG.Mongo.Uri(),
		Coll:             global.MALL_CONFIG.Mongo.Coll,
		Database:         global.MALL_CONFIG.Mongo.Database,
		MinPoolSize:      &global.MALL_CONFIG.Mongo.MinPoolSize,
		MaxPoolSize:      &global.MALL_CONFIG.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &global.MALL_CONFIG.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &global.MALL_CONFIG.Mongo.ConnectTimeoutMs,
		Auth: &qmgo.Credential{
			Username:   global.MALL_CONFIG.Mongo.Username,
			Password:   global.MALL_CONFIG.Mongo.Password,
			AuthSource: global.MALL_CONFIG.Mongo.AuthSource,
		},
	}, opts...)
	if err != nil {
		return errors.Wrap(err, "链接mongodb数据库失败!")
	}
	global.MALL_MONGO = client
	err = m.Indexes(ctx)
	if err != nil {
		return err
	}
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := global.MALL_MONGO.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = global.MALL_MONGO.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
