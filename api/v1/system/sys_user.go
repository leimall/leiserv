...
func (b *BaseApi) TokenNext(c *gin.Context, user system.SysUser) {
	j := &utils.JWT{SigningKey: []byte(global.MALL_CONFIG.JWT.SigningKey)} // 唯一签名
...
func (b *BaseApi) Register(c *gin.Context) {
	...
	if err != nil {
		global.MALL_LOG.Error("注册失败!", zap.Error(err))
...
func (b *BaseApi) ChangePassword(c *gin.Context) {
	...
	u := &system.SysUser{DATE_MODEL: global.DATE_MODEL{ID: uid}, Password: req.Password}
	_, err = userService.ChangePassword(u, req.NewPassword)
	if err != nil {
		global.MALL_LOG.Error("修改失败!", zap.Error(err))
...
func (b *BaseApi) SetUserInfo(c *gin.Context) {
	...
	if len(user.AuthorityIds) != 0 {
		...
		if err != nil {
			global.MALL_LOG.Error("设置失败!", zap.Error(err))
			response.FailWithMessage("设置失败", c)
			return
		}
	}
	err = userService.SetUserInfo(system.SysUser{
		DATE_MODEL: global.DATE_MODEL{
			ID: user.ID,
		},
		NickName:  user.NickName,
...
func (b *BaseApi) ResetPassword(c *gin.Context) {
	...
	if err != nil {
		global.MALL_LOG.Error("重置失败!", zap.Error(err))
