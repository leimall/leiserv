...
func (autoApi *AutoCodeApi) PreviewTemp(c *gin.Context) {
	...
	if err != nil {
		global.MALL_LOG.Error("预览失败!", zap.Error(err))
...
func (autoApi *AutoCodeApi) CreateTemp(c *gin.Context) {
	...
	if a.AutoCreateApiToSql {
		if ids, err := autoCodeService.AutoCreateApi(&a); err != nil {
			global.MALL_LOG.Error("自动化创建API失败!", zap.Error(err))
	...
	if a.AutoCreateMenuToSql {
		if id, err := autoCodeService.AutoCreateMenu(&a); err != nil {
			global.MALL_LOG.Error("自动化创建菜单失败!", zap.Error(err))
			c.Writer.Header().Add("success", "false")
...
func (autoApi *AutoCodeApi) GetDB(c *gin.Context) {
	...
	for _, db := range global.MALL_CONFIG.DBList {
	...
	if err != nil {
		global.MALL_LOG.Error("获取失败!", zap.Error(err))
...
func (autoApi *AutoCodeApi) GetTables(c *gin.Context) {
	dbName := c.DefaultQuery("dbName", global.MALL_CONFIG.Mysql.Dbname)
	businessDB := c.Query("businessDB")
	tables, err := autoCodeService.Database(businessDB).GetTables(businessDB, dbName)
	if err != nil {
		global.MALL_LOG.Error("查询table失败!", zap.Error(err))
...
func (autoApi *AutoCodeApi) CreatePackage(c *gin.Context) {
	...
	if err != nil {
		...
		global.MALL_LOG.Error("创建失败!", zap.Error(err))
...
func (autoApi *AutoCodeApi) DelPackage(c *gin.Context) {
	...
	if err != nil {
		global.MALL_LOG.Error("删除失败!", zap.Error(err))
...
func (autoApi *AutoCodeApi) LLMAuto(c *gin.Context) {
	...
	params["mode"] = mode
	path := strings.ReplaceAll(global.MALL_CONFIG.AutoCode.AiPath, "{FUNC}", "api/chat/ai")
	...
	if err != nil {
		global.MALL_LOG.Error("大模型生成失败!", zap.Error(err))
	...
	if resStruct.Code == 7 {
		global.MALL_LOG.Error("大模型生成失败!"+resStruct.Msg, zap.Error(err))
...
func (autoApi *AutoCodeApi) PubPlug(c *gin.Context) {
	...
	if err != nil {
		global.MALL_LOG.Error("打包失败!", zap.Error(err))
