...
import (
	...
	exampleRes "leiserv/models/example/response"
	"leiserv/utils"
...
func (b *FileUploadAndDownloadApi) BreakpointContinue(c *gin.Context) {
	...
	if err != nil {
		global.MALL_LOG.Error("接收文件失败!", zap.Error(err))
		response.FailWithMessage("接收文件失败", c)
		return
	}
	f, err := FileHeader.Open()
	if err != nil {
		global.MALL_LOG.Error("文件读取失败!", zap.Error(err))
...
func (b *FileUploadAndDownloadApi) FindFile(c *gin.Context) {
	...
	file, err := fileUploadAndDownloadService.FindOrCreateFile(fileMd5, fileName, chunkTotal)
	if err != nil {
		global.MALL_LOG.Error("查找失败!", zap.Error(err))
...
func (b *FileUploadAndDownloadApi) BreakpointContinueFinish(c *gin.Context) {
	...
	if err != nil {
		global.MALL_LOG.Error("文件创建失败!", zap.Error(err))
...
func (b *FileUploadAndDownloadApi) RemoveChunk(c *gin.Context) {
	...
	if err != nil {
		global.MALL_LOG.Error("缓存切片删除失败!", zap.Error(err))
		return
	}
	err = fileUploadAndDownloadService.DeleteFileChunk(file.FileMd5, file.FilePath)
	if err != nil {
		global.MALL_LOG.Error(err.Error(), zap.Error(err))
		response.FailWithMessage(err.Error(), c)
		return
	}
